...
#define IRQBIT(irq)		BIT(((irq)-1) & 0x1f)
#define IRQWD(irq)		(base + ((((irq)-1) >> 5) << 2))
...
#define do_IRQ(x)		handle_IRQ((x), regs)
...
asmlinkage void __exception_irq_entry brcmstb_handle_irq(struct pt_regs *regs)
{
	...
	pend = __raw_readl(IRQWD(1) + 0x00) & ~__raw_readl(IRQWD(1) + 0x0c);
	...
	pend = __raw_readl(IRQWD(33) + 0x00) & ~__raw_readl(IRQWD(33) + 0x0c);
	while ((shift = ffs(pend)) != 0) {
		pend ^= (1 << (shift - 1));
		shift += 32;
		do_IRQ(shift);
	...
	pend = __raw_readl(IRQWD(65) + 0x00) & ~__raw_readl(IRQWD(65) + 0x0c);
	while ((shift = ffs(pend)) != 0) {
		pend ^= (1 << (shift - 1));
		shift += 64;
		do_IRQ(shift);
...
void __init brcmstb_init_irq(void)
{
	...
	for (n = 1; n <= L1_IRQS; n += 32) {
		__raw_writel(0xffffffff, IRQWD(n) + 0x18);

		for (i = n; (i < (n + 32)) && (i <= L1_IRQS); i++) {
